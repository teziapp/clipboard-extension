...
const AddOrEditSnippet = () => {
    const { snippets, addSnippet, updateSnippet, deleteSnippet, tags: allTags, addTag, isDarkMode } = useSnippets();
    ...
    return (
        <div className={`p-4 h-full flex flex-col overflow-y-auto ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white'}`}>
            ...
            <textarea
                className={`w-full h-40 p-3 mb-4 rounded-lg resize-none ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}
                value={content}
            ...
            <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                    {snippetTags.map((tag, index) => (
                        <span key={index} className={`px-3 py-1 rounded-full text-sm flex items-center ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                            {tag}
                            <button
                ...
                <div className="flex items-center">
                    <input
                        type="text"
                        className={`flex-grow p-2 rounded-l-lg ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}
                        value={newTag}
                    ...
                    <button
                        className={`p-2 rounded-r-lg text-blue-500 text-xl h-full ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                        ...
                        <div className="flex justify-end">
                            <button 
                                className={`mr-2 px-4 py-2 rounded ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-gray-200 text-black'}`}
